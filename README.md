...
Figure below explains the principle behind how K-means clustering group the pixels: 
...
## LULC Classification Project Video 

[![Watch the video](https://img.youtube.com/vi/BGCmyY5JUKU/0.jpg)](https://youtu.be/BGCmyY5JUKU)

## 1. Data Collection

Level-1 Ocean and Land Colour Instrument (OLCI) imageries were acquired from **ESA Sentinel-3** for the area of interest. Data were retrieved using the Copernicus Data Space Ecosystem API, with filtering by geographic bounding box and acquisition date. Image details are as follow:
...
plt.show()
```
NDVI visual of the region of interest:
...
Before machine learning could be deployed, data must be made NumPy array and cleaned to remove NaN values
```sh
...
## 3. Deploying machine learning models to classify LULC
...
Since the output clusters are arbitrary (meaningless colour scheme), we need to manually re-map the cluster IDs to meaningful land cover classes and assign corresponding colors:
- Green: Vegetation
- Blue: Water
- Grey: Cloud
...
### Qualitative Assessment (Side-by side visual)
...
## LULC Classification on the second image
...
## 🍃 Environmental impact assessment of this project
